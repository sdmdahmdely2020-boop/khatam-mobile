...
- ✅ **Accueil élève : conversion + motivation, Étape 1 du plan de croissance (29/08)** — voir `khatam-strategie-croissance-29-08.md` (dans le projet Claude) pour le plan complet en 6 volets ; ceci est UNIQUEMENT l'étape 1 (accueil élève), les autres volets (professeurs, monétisation, admin) restent à valider avant d'être construits :
  - Message de valeur : bouton/bandeau "Réussir mon Bac 🚀" à la place de "Devenir Premium", sous-titre "Accès illimité aux TD et corrigés, sans publicité."
  - Paywall plus clair : étiquette "🔒 Inclus dans Premium" sur les documents verrouillés (en plus du cadenas déjà existant).
  - **Upsell intelligent** : toucher un document verrouillé ouvre d'abord une fenêtre "Ce document peut t'aider à réussir ton examen 🎯" avec le prix (ou "Inclus dans Premium") et deux choix clairs — jamais bloquant, l'élève peut toujours continuer vers le document.
  - **Économies réelles** : pour un abonné Basic, "Vous avez économisé X MRU" calculé à partir des achats réels du mois (jamais un chiffre inventé — voir note technique) ; pour un Premium, un message de valeur ("Accès illimité activé") plutôt qu'un montant qu'on ne peut pas mesurer honnêtement.
  - **Motivation** : série de jours d'affilée (🔥, calculée sur cet appareil uniquement), objectif hebdomadaire (🎯 "X/2 documents cette semaine"), progression par matière (déjà existante, conservée).
  - **Notifications — structure uniquement**, comme demandé explicitement : nouvel écran (icône cloche dans l'accueil), modèle de données prêt (nouveau document, rappel de progression, abonnement qui expire), mais TOUJOURS vide pour l'instant — pas de fausses notifications, pas encore de vrai système de déclenchement côté serveur (piste pour plus tard).
...
- **Série de jours (streak, 29/08)** : calculée et stockée UNIQUEMENT sur l'appareil (`SharedPreferences`, pas de route backend) — se réinitialise si l'élève change d'appareil ou réinstalle l'app. Un vrai suivi multi-appareils demanderait une route dédiée côté serveur, volontairement pas construite maintenant pour rester dans le périmètre "aucun changement backend" de l'étape 1.
- **Économies (29/08)** : calculées UNIQUEMENT à partir de vrais achats confirmés (`GET /api/documents/mine`) — jamais estimées ni inventées. Conséquence assumée : un abonné Premium n'a pas de montant "économisé" affiché (aucune donnée d'achat individuel une fois Premium actif), remplacé par un message de valeur. Voir `lib/features/motivation/models/motivation_summary.dart`.
